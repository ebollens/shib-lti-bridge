...
class BLTI {
    ...
    function getUserImage() {
        $image = isset($this->info['user_image']) ? $this->info['user_image'] : false;
        if ( $image && strlen($image) > 0 ) return $image;
        $email = $this->getUserEmail();
        if ( $email === false ) return false;
        $size = 40;
        $grav_url = !empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https://' : 'http://';
